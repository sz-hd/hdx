...
public abstract class ModelToInsert {
	 ...
	 * @throws Exception 
	 */
	public static synchronized Map<String, Object> convertInsertSql(Object model, Map<String, Object> parameter, String prexix) throws Exception {
		...
		sqlInsert.append(table.name().replaceAll("\\{PREFIX\\}", prexix));
		sqlInsert.append(" ( ");
		...
		for (Field field : fields) {
			...
			if (column != null) {
				if(parameter!=null){
					...
					ModelToSqlValue.checkRequiredByNull(column, value); 
					if (value != null) {
						convertSql(sqlInsert,sqlValue,column,isqlInsert,isqlValue);
