...
import com.hdx.common.exception.UtilException;
...
public class EncryptBase64Util {
    ...
    public static byte[] decode(String src) throws Exception {  
        ...
        if(buf.length() % 4 != 0) {  
            throw new UtilException("Base64 decoding invalid length");  
        }  
        ...
        for(i = 0; i < buf.length(); i+=4) {  
            ...
            for(int j = 0; j < 4; j++) {  
                ...
                if(data == -1) {  
                    throw new UtilException("Base64 decoding bad character");  
                }  
