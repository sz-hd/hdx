package com.hdx.common.dao.impl;
...
import com.hdx.common.dao.BaseDao;
import com.hdx.common.dao.Jdbc;
import com.hdx.common.dao.ModelToDelete;
import com.hdx.common.dao.ModelToInsert;
import com.hdx.common.dao.ModelToSelect;
import com.hdx.common.dao.ModelToUpdate;
import com.hdx.common.dao.ResultSetToModel;
import com.hdx.common.exception.ModelException;
import com.hdx.common.pageHelper.IPage;
import com.hdx.common.pageHelper.impl.PageHelper;
 ...
 *
 */
public class BaseDaoImpl extends Jdbc implements BaseDao {
	@Autowired
...
//		Long startTime = StatisticalTime.startTime();// 统计执行时间 开始
		Long count = (Long) jdbcTemplate.query(sql, args, new ResultSetExtractor<Long>() {
			@Override
...
//			agrsString += item.toString()+",";
//		} 
//		System.out.println("查询总数："+sql+",\n参数"+agrsString+"\n 共消耗:"+(endTime -startTime)+"ms");
//		StatisticalTime.seveTimeLogToMongo(startTime, endTime, sql);// 记录
	...
	protected String getFirstValue(String sql, String defVaule) {
	    String value = (String) jdbcTemplate.query(sql,new ResultSetExtractor<String>() {
		      @Override
	...
	protected String getFirstValueByArgs(String sql,Object[] args, String defVaule) {
	    String value = (String) jdbcTemplate.query(sql, args ,new ResultSetExtractor<String>() {
		      @Override
		      public String extractData(ResultSet rs) throws SQLException, DataAccessException {
			      if (rs.next())
			    	  return rs.getString(1);
	 ...
	 * @throws Exception
	 */
	public  int insertDynamic(Object model,Map<String,Object> parameter, String prefix) throws Exception {
		if (parameter != null) {
			TimeZone tz = TimeZone.getTimeZone("Asia/Shanghai");
			TimeZone.setDefault(tz);
			if (!parameter.containsKey("createDate")) {
...
//		Long time1 = System.currentTimeMillis();
		int results = this.jdbcTemplate.update(sql, args);
//		String argsString ="";
//		for(Object item:args){
...
//		StatisticalTime.seveTimeLogToMongo(startTime, endTime, sql);// 记录
	...
	@Override
	public List<Object> queryDynamic(Object model, Map<String, Object> parameter, String prefix) throws Exception {
		Map<String, Object> map = ModelToSelect.convertSelectSql(model, parameter,prefix);
		return query(map.get("sql").toString(), ((ArrayList<?>) map.get("para")).toArray(), model.getClass());
	}
...
//		Long startTime = StatisticalTime.startTime();// 统计执行时间 开始
		...
		if (classz.equals(HashMap.class)) {
			list = (List<Object>) jdbcTemplate.query(sql, args, new ResultSetExtractor() {
				@Override
...
//		System.out.println("");
	...
	public Map<String, Object> queryDataListByMap(String sql) throws Exception {		
		@SuppressWarnings({ "unchecked" })
		List<Map<String, Object>> list = (List<Map<String, Object>>) jdbcTemplate.query(sql,
