...
public class BaseDaoOfMysqlImpl extends Jdbc implements BaseDao {
	 ...
	 * @return
	 */
	protected IPage getPage(int _totalrows, int _currentPage, int _pageSize, int type) {
		return new PageHelper(_totalrows, _currentPage, _pageSize, type).getPage();
	}
	...
	public int insertDynamic(Object model, Map<String, Object> parameter, String prefix) throws Exception {
		...
		Map<String, Object> map = ModelToInsert.convertInsertSql(model, parameter, prefix);
...
//		Long startTime = StatisticalTime.startTime();// 统计执行时间 开始
	...
	public Map<String, Object> queryNavigator(String sql, Object[] args, int _currentPage, int _pageSize, Object model)
		...
		IPage page = getPage(Integer.valueOf(count.toString()), _currentPage, _pageSize,0);
...
//		sqlStr.append(" where  table2.num between ? and ? ");
		...
		sqlStr.append(" select table2.* from ( ");
		sqlStr.append(sql.toString());
		sqlStr.append(" ) table2 ");
		sqlStr.append(" limit ?,? ");
	...
	public String getSqlByPageSize(String sql, int _currentPage, int _pageSize) {
		IPage page = getPage(0, _currentPage, _pageSize,0);
